...
def fpn_classifier_graph(rois, feature_maps,
                         image_shape, pool_size, num_classes,num_keypoints = 17,train_bn=True):
    ...
    x = KL.TimeDistributed(BatchNorm(axis=3), name='mrcnn_class_bn1')(x,training=train_bn)
    x = KL.Activation('relu')(x)
...
def _timedistributed_depthwise_conv_block(inputs, pointwise_conv_filters, strides=(1, 1), block_id=1, train_bn=False):
    ...
    # Depthwise
    x = KL.TimeDistributed(DepthwiseConv2D(
                    (3, 3),
                    padding='same',
                    depth_multiplier=1,
                    strides=strides,
...
def _kytimedistributed_depthwise_conv_block(inputs, pointwise_conv_filters, strides=(1, 1), block_id=1, train_bn=False):
    ...
    x = KL.TimeDistributed(DepthwiseConv2D(
                    ...
                    strides=strides,
                    use_bias=False),
                    name='mrcnn_mask_conv__kydw_{}'.format(block_id))(inputs)
    x = KL.TimeDistributed(BatchNorm(axis=channel_axis),
                    name='mrcnn_mask_conv_kydw_{}_bn'.format(block_id))(x, training=train_bn)
    x = KL.Activation(relu6, name='mrcnn_mask_conv_kydw_{}_relu'.format(block_id))(x)
    # Pointwise
    x = KL.TimeDistributed(KL.Conv2D(pointwise_conv_filters,
                    (1, 1),
                    padding='same',
                    use_bias=False,
                    strides=(1, 1)),
                    name='mrcnn_mask_conv_kypw_{}'.format(block_id))(x)
    x = KL.TimeDistributed(BatchNorm(
                    axis=channel_axis),
                    name='mrcnn_mask_conv_kypw_{}_bn'.format(block_id))(x, training=train_bn)
    return KL.Activation(relu6, name='mrcnn_mask_conv_kypw_{}_relu'.format(block_id))(x)


def build_fpn_keypoint_graph(rois, feature_maps,
                         image_shape, pool_size, num_keypoints, backbone, train_bn=False):
    ...
    if backbone in ['resnet50','resnet101']:
        for i in range(8):
            ...
            x = KL.Activation('relu')(x)
    if backbone in ['mobilenetv1','mobilenetv2']:
        x = _kytimedistributed_depthwise_conv_block(x, 512, block_id = 1, train_bn = train_bn)
        x = _kytimedistributed_depthwise_conv_block(x, 512, block_id = 2, train_bn = train_bn)
        x = _kytimedistributed_depthwise_conv_block(x, 512, block_id = 3, train_bn = train_bn)
        x = _kytimedistributed_depthwise_conv_block(x, 512, block_id = 4, train_bn = train_bn)
        x = _kytimedistributed_depthwise_conv_block(x, 512, block_id = 5, train_bn = train_bn)
        x = _kytimedistributed_depthwise_conv_block(x, 512, block_id = 6, train_bn = train_bn)
        x = _kytimedistributed_depthwise_conv_block(x, 512, block_id = 7, train_bn = train_bn)
        x = _kytimedistributed_depthwise_conv_block(x, 512, block_id = 8, train_bn = train_bn)
